...
RUN_FILE = PROJECT_ROOT / "data" / "Evalcase03_RerankRun01.xlsx"
OUTPUT_FILE = PROJECT_ROOT / "results" / "Evalcase03_RerankRun01_results.xlsx"
...
for index, row in run.iterrows():
    ...
    # First stage: retrieve Top 10 with embeddings
    top10_indices = (
        embedding_scores
    ...
    candidate_docs = corpus.iloc[top10_indices]

    # Second stage: rerank Top 10 with CrossEncoder
    pairs = [
        (question, text)
        for text in candidate_docs["search_text"].tolist()
    ]
    ...
    final_indices = [
        top10_indices[i]
        for i in reranked_positions
    ]

    top_docs = (
        corpus.iloc[final_indices]["doc_id"]
        .astype(str)
        .tolist()
    )
...
print("Embedding + reranking retrieval complete.")
